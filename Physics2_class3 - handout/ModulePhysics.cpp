...
bool PhysBody::Contains(int x, int y) const
{
	...
	b2Fixture* iterator = body->GetFixtureList();
	b2Vec2 point = { PIXEL_TO_METERS(x), PIXEL_TO_METERS(y) };

	while (iterator != NULL) {
		
		if (iterator->TestPoint(point))
			ret = true;
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	b2Fixture* iterator = body->GetFixtureList();
	b2Vec2 point1 = { PIXEL_TO_METERS(x1), PIXEL_TO_METERS(y1) };
	b2Vec2 point2 = { PIXEL_TO_METERS(x2), PIXEL_TO_METERS(y2) };
	...
	while (iterator != NULL) {
		...
		if (iterator->RayCast(&output, input, childIndex)) {
			collision.x = (point2.x - point1.x) * output.fraction + point1.x;
			collision.y = (point2.y - point1.y) * output.fraction + point1.y;

			normal_x = collision.x - point1.x;
			normal_y = collision.y - point1.y;

			ret = abs((collision - point1).LengthSquared());
		}
			iterator = iterator->GetNext();
	}
