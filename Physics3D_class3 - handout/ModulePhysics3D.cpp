...
bool ModulePhysics3D::CleanUp()
{
	...
	p2List_item<btCollisionShape*>* item_col = collisions.getFirst();
	while (item_col->next != nullptr) {
		delete item_col;
		item_col = item_col->next;
	}
	collisions.clear();

	p2List_item<btRigidBody*>* item_body = rigid_bodies.getFirst();
	while (item_body->next != nullptr) {
		delete item_body;
		item_body = item_body->next;
	}
	rigid_bodies.clear();
